namespace :csv_import do
  require 'csv'
  desc "task to import csv import to database"
  task import_new_tutors: :environment do
    csv_text = File.read(Rails.root.join('lib', 'seeds', 'new-volunteers-registration.csv'))
    ...
    csv.each do |row|
      ...
      # This is for declaration form data
      t.declare_1 = row['Have you ever been charged in a Court of Law in Singapore or in any other country?'] == 'No' ? true : false
      t.declare_2 = row['Have you ever been involved in care proceedings or in a child protection investigation or enquiry in Singapore or any other country? '] == 'No' ? true : false
      t.declare_3 = row['Are you previously or currently known to the Family Court for Personal Protection Order/ Expedited Order or family violence issues in Singapore or any other country? '] == 'No' ? true : false
      t.declare_4 = row['Have you been or are you under any financial embarrassment i.e. (a) an undischarged bankrupt, (b) a judgement debtor, (c) have unsecured debts and liabilities of more than 3 months of last-drawn pay, (d) have signed a promissory note or an acknowledgement of indebtedness?'] == 'No' ? true : false
      # This is for importing subjects
      t.subjects = {
        primary_subjects: row['Subjects (Primary)'].present? ? row['Subjects (Primary)'] : '',
        secondary_subjects: [ row['Subjects N(T) Level'], row['Subjects N(A) '], row['Subjects - O Level '] ].join(', ').present? ? [ row['Subjects N(T) Level'], row['Subjects N(A) '], row['Subjects - O Level '] ].join(', ') : '' ,
        jc_subjects: [
          row['Languages [Chinese Language (H1 only)]'].present? ? 'Chinese language(h1 only)' : '',
          row['Languages [General Studies in Chinese (H1 only)]'].present? ? 'General Studies in Chinese (H1 only)' : '',
          row['Languages [Malay Language (H1 only)]'].present? ? 'Malay Language (h1 only)' : '',
          row['Languages [Tamil Language (H1 only)]]'].present? ? 'Tamil Language (H1 only)]' : '',
          row['Languages [Bengali (H1 only)]'].present? ? 'Bengali (H1 only)' : '',
          row['Languages [Gujarati (H1 only)]'].present? ? 'Gujarati (H1 only)' : '',
          row['Languages [Hindi (H1 only)]'].present? ? 'Hindi (h1 only)' : '',
          row['Languages [French ]'].present? ? 'French (h1)' : '',
          row['Languages [German]'].present? ? 'German (h1)' : '',
          row['Languages [Japanese]'].present? ? 'Japanese (h1)' : '',
          row['Languages [Spanish (H2 only)]'].present? ? 'Spanish (H2 only)' : '',
          row['Humanities [General Paper (H1 only)]'].present? ? 'General Paper (h1)' : '',
          row['Humanities [Knowledge and Inquiry (H2 only)]'].present? ? 'Knowledge and Inquiry (h2)' : '',
          row['Humanities [China Studies in English ]'].present? ? 'China studies in english (h1)' : '',
          row['Humanities [China Studies in Chinese (H2 only)]'].present? ? 'China studies in chinese (h2 only)' : '',
          row['Humanities [Geography]'].present? ? 'Geography (h2)' : '',
          row['Humanities [History]'].present? ? 'History (h2)' : '',
          row['Humanities [Economics]'].present? ? 'Economics (h2)' : '',
          row['Humanities [Art]'].present? ? 'Art (h2)' : '',
          row['Humanities [Music]'].present? ? 'Music (h2)' : '',
          row['Humanities [Literature in English]'].present? ? 'Literature in English (h1)' : '',
          row['Humanities [English Language and Linguistics (H2 only)]'].present? ? 'English lang and linguistics (h2 only)' : '',
          row['Humanities [Chinese Language and Literature (H2 only)]'].present? ? 'Chinese language and lit (h2 only)' : '',
          row['Humanities [Malay Language and Literature (H2 only)]'].present? ? 'Malay language and lit (h2 only)' : '',
          row['Humanities [Tamil Language and Literature (H2 only)]'].present? ? 'Tamil language and lit (h2 only)' : '',
          row['Humanities [Translation (Chinese) (H2 only)'].present? ? 'Translation(Chinese) (h2 only)' : '',
          row['Sciences, Mathematics, Business and Computing [Mathematics]'].present? ? 'Math (h2)' : '',
          row['Sciences, Mathematics, Business and Computing [Further Mathematics (H2 only)]'].present? ? 'Further Mathematics (h2)' : '',
          row['Sciences, Mathematics, Business and Computing [Physics]'].present? ? 'Physics (h2)' : '',
          row['Sciences, Mathematics, Business and Computing [Chemistry]'].present? ? 'Chemistry (h2)' : '',
          row['Sciences, Mathematics, Business and Computing [Biology]'].present? ? 'Biology (h2)' : '',
          row['Sciences, Mathematics, Business and Computing [Computing (H2 only)]'].present? ? 'Computing (h2 only)' : '',
          row['Sciences, Mathematics, Business and Computing [Management of Business (H2 only)]'].present? ? 'Management of Business (h2 only)' : '',
          row['Sciences, Mathematics, Business and Computing [Principles of Accounting (H2 only)]'].present? ? 'Principle of accounts (h2 only)' : ''
        ].join(', ')
      }
      t.save
